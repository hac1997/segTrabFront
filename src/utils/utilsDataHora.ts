...
export function tempoRestanteParaVencer(tarefa: Tarefa): string {

  const [day, month, year] = tarefa.data.split('/').map(Number);
  const [hour, minute] = tarefa.hora.split(':').map(Number);

  const dataTarefa: Date = new Date(year, month - 1, day, hour, minute);
  const agora: Date = new Date();

  const diffMs = dataTarefa.getTime() - agora.getTime();

  if (diffMs <= 0) { return 'Já venceu'; }
  ...
  if (horas > 0 && minutos > 0) { resultado += ' e '; }
...
export function parseData(dataString: string): number[] {
...
export function parseDataHora(dataStr: string, horaStr: string): Date {
  const [dia, mes, ano] = dataStr.split('/').map(Number);
  const [hora, minuto] = horaStr.split(':').map(Number);
  return new Date(ano, mes - 1, dia, hora, minuto);
}

export function parseDataBrasil(dataStr:string):Date|null {
...
export function getAnoSemana(data:Date):string {
...
export function validaData(dataString: string): boolean {
...
export function validaHora(horaString: string): boolean {
  const regex = /^(\d{2}):(\d{2})$/;
...
export function dataExpirada(data: string, hora: string): boolean {
